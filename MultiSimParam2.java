...
class MultiSimParam2 {
    ...
    // on suppose que les fichiers de paramtres ont l'extension .txt

 MultiSimParam2(String fileName, int nLDebut, int nLFin, int fs) {
        boolean entete = true ;
        for (int i = nLDebut; i <= nLFin; i++) {
            new ExperimentalDesignOrTrajectory(fileName, i, fs, entete) ;
            entete=false ;
    ...
    MultiSimParam2(String fileName, String nomFichierSortie, int nLDebut, int nLFin, int fs) {
        boolean entete =true ;
        for (int i = nLDebut; i <= nLFin; i++) { //read from nLDebut until nLfin, in our case we have only one line 
            new ExperimentalDesignOrTrajectory(fileName, nomFichierSortie, i, fs, entete) ;
            entete=false ;
            System.err.println("jeux de parametres n� " + (i - nLDebut + 1)
                    + " fini");
        }
